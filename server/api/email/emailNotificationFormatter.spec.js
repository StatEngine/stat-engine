...
describe('Email Notification Formatter', () => {
  ...
  it('should execute without errors', async () => {
    ...
    const sourceShell = fs.readFileSync(`${process.cwd()}/server/api/email/templates/shell.hbs`, 'utf-8');
    const templateShell = Handlebars.compile(sourceShell);

    const sourcePartial = fs.readFileSync(`${process.cwd()}/server/api/email/templates/event.hbs`, 'utf-8');
    Handlebars.registerPartial('event', sourcePartial);


    const html = templateShell(mergeVars);
    console.log(html);
  });
});

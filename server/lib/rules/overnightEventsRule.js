...
export class OvernightEventsRule extends Rule {
  ...
  finalReportDetails(reportDetails, params) {
    const analysis = [];

    _.chunk(reportDetails, params.reportChunkSize)
      .forEach(detail => {
        analysis.push({
          rule: params.reportRuleName,
          level: params.reportLevel,
          description: params.reportDescription,
          default_visibility: params.reportDefaultVisibility,
          detailList: detail,
        });
      });
    return analysis;

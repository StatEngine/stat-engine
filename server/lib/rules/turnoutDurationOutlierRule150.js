...
export class TurnoutDurationOutlierRule150 extends Rule {
  constructor(params) {
    super(params);
    this.params.threshold = this.params.threshold || 150;
  ...
  analyze() {
    ...
    this.results.hits.hits.forEach(hit => {
      ...
      hit._source.apparatus.forEach(u => {
        const turnout_duration = _.get(u, 'extended_data.turnout_duration');
...
export default { TurnoutDurationOutlierRule150 };

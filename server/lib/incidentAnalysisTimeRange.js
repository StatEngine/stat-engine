...
export class IncidentAnalysisTimeRange {
  ...
  eventDurationSumRuleAnalysis() {
    return this.analyze([EventDurationSumRule]);
  }
  ...
  ruleAnalysis() {
    ...
    return this.analyze(ruleConfig);
  }
  ...
  analyze(ruleConfig) {
    ...
    ruleConfig.forEach(rule => {
      ...
      rules.push(myRule);
      const query = myRule.query.filter('range', 'description.event_opened', {
        gte: this.currentTimeFilter.start,
        lt: this.currentTimeFilter.end,
      })
        .build();
      queries.push({});
      queries.push(query);
